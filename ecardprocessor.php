...
include_once('classes/ahaecardgenclass.php');
    ...
    //include_once('admin/commissionhandler.php');
...
$partial=200;///incase of partial payment
							...
							else if($actual_balance<$partial)
							{
								...
								else if($_SESSION['total_ecard_value']==$partial)
								{
								   ...
								   array_push($_SESSION['affected_ecards_used'],$actual_balance);
									$new_balance=0;
									$ok_to_order=1;
									$ermsg="you have paid the necessary amount, you can now order";
									if(isset($_SESSION['total_ecard_value']))
								...
								{
									$_SESSION['has_paid']=1;
									$new_balance=$_SESSION['total_ecard_value']-$partial;
									...
									$ermsg="you have paid the necessary amount, and the last ecard you entered has a remaining amount of '$new_balance',please order now";
									if(isset($_SESSION['total_ecard_value']))
									unset($_SESSION['total_ecard_value']);
			    ...
			    {		
					...
					if(count_days($today,$generation_date)>180)
				...
				else
						{
							...
							else if($actual_balance<$total)
							{
							
								$_SESSION['total_ecard_value']=($_SESSION['total_ecard_value'])+$actual_balance;
								if(($_SESSION['total_ecard_value'])<$total)
								{
									 array_push($_SESSION['affected_ecards_id'],$eid);
								   array_push($_SESSION['affected_ecards_used'],$actual_balance);
									$new_balance=0;
									//update_ecard($ecardnum,$pincode,$actual_balance);
									$total_paid=$_SESSION['total_ecard_value'];
									$remaining_amt=$total-($_SESSION['total_ecard_value']);
									$ermsg="you have paid $total_paid and the remaining amount is $remaining_amt, please enter another card again";	
								}
								else if($_SESSION['total_ecard_value']==$total)
								{
									$_SESSION['has_paid']=1;
								   array_push($_SESSION['affected_ecards_id'],$eid);
								   array_push($_SESSION['affected_ecards_used'],$actual_balance);
								...
								{
									$_SESSION['has_paid']=1;
									$new_balance=$_SESSION['total_ecard_value']-$total;
									$amount_used=$actual_balance-$new_balance;
									//$updateused=$amount_used+$used;
									//array_push($_SESSION['affected_ecards'],$eid,$amount_used);
									 array_push($_SESSION['affected_ecards_id'],$eid);
									 array_push($_SESSION['affected_ecards_used'],$amount_used);
									$ok_to_order=1;
									...
									$ermsg="you have paid the necessary amount, and the last ecard you entered has a remaining amount of '$new_balance', you can now order";
									if(isset($_SESSION['total_ecard_value']))
									unset($_SESSION['total_ecard_value']);
								}
							...
							//$ermsg="your card only has a value of '$actual_balance 'please insert a card with a value greater or equal to '$total'";
							}
							else if($actual_balance>=$total)
							{
							$_SESSION['has_paid']=1;
							//$updateused=$total;
							//$new_balance=$actual_balance-$total;
							//$row["lover"]=$row["lover"]-150;
							//$query="update ecard set used='$updateused' where ecardnum='$ecardnum'";
							$paid_sofar=$_SESSION['total_ecard_value'];
							$due=$total-$paid_sofar;
							$new_balance=$actual_balance-$due;
							$ok_to_order=1;
							
							
						    array_push($_SESSION['affected_ecards_id'],$eid);
							array_push($_SESSION['affected_ecards_used'],$due);
			               ...
			               }// end of the expiry date checker else
	                }///               no double entry of an ecard
...
<html>
<head>
<title><?php echo SYSTEM_NAME;?>-Payment</title>
...
<body >
<TABLE width=987 height="100%" border=0 align="center">
  <TBODY>
    <TR> 
      <!----Header Links---->
...
function delete_cookie(cookie_name){
    ...
    document.cookie = cookie_name + "=''; expires=" + expdate.toGMTString();
      ...
      <TD width="15%" rowspan="2" align=middle vAlign=top> <TABLE width=157 border=0>
          <TBODY>
            <TR></TR>
            <TR></TR>
            <TR></TR>
            ...
            align=left border=0>
                ...
                onmouseout="this.background=''">You can choose the kind of payment 
                        you want to make.If you choose to make partial payment, 
                        you will complete payment from your commissions.&nbsp;&nbsp;&nbsp;</TD>
                    </TR>
                  </TBODY>
                </TABLE></TD>
            </TR>
    ...
    <TR>
      <TD align=left><TABLE height=514 width=709 border=0>
          <TBODY>
            <TR> 
              <TD vAlign=top><form action="ecardprocessor.php" method="post" name="ecardprocessor" id="ecardprocessor">
                  <table width="104%" border="0" align="center">
                    <tr> 
                      <td width="21%" id="main_content">Payment options:</td>
                      <td colspan="5"><select name="payment_option" id="payment_option">
                          <option value="partial">Partial</option>
                          <option value="full" selected>Full</option>
                        </select>
                        <font color="#990000" size="2">* Make sure to choose the right 
                        type of payment for each ecard entry</font></td>
                    </tr>
                    <tr bgcolor="#CCCCCC"> 
                      <td height="25" id="main_content">Ecard code</td>
 ...
 elseif(isset($_POST['order']))
	    {
		 if($_SESSION['has_paid']==1)		
		    {
		    update_ecard($_SESSION['affected_ecards_id'],$_SESSION['affected_ecards_used']);
			//while ( list( $eid, $amt ) = each($_SESSION['affected_ecards'] ) )
            //echo "$eid - $amt<br />";//echo $_SESSION['affected_ecards']; 
		     header("location:receipt.php");
			 if(isset($cart))
...
function delete_cookie(cookie_name){
    ...
    document.cookie = cookie_name + "=''; expires=" + expdate.toGMTString();
          ...
          <TBODY>
            ...
            align=left border=0>
                  <TBODY>
                    <TR align=left> 
                      <TD onMouseOver="this.background='images/select.jpg'" 
                onmouseout="this.background=''">The product you have chose to 
                        order can only be paid fully.So you will have to enter 
                        an ecard with a value greater or equal to the price of 
                        the product!</TD>
                    </TR>
                  </TBODY>
                </TABLE></TD>
      ...
      <TD align=left><font size="4"><strong>Ahha e-card Payment</strong></font></TD>
    </TR>
    <TR>
      <TD align=left><TABLE height=514 width=709 border=0>
          <TBODY>
            <TR> 
              <TD vAlign=top><form action="ecardprocessor.php" method="post" name="ecardprocessor" id="ecardprocessor">
                  <table width="104%" border="0" align="center">
                    <tr> 
                      <td colspan="6" id="main_content"> <font color="#990000" size="2">*please 
                        enter an ecard with a value greater than or equal to<?php echo $total;?></font></td>
                    </tr>
                    <tr bgcolor="#CCCCCC"> 
